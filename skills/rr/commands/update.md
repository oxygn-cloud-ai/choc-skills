...
allowed-tools: Read, Bash(git *), Bash(bash install.sh *), Bash(bash ./install.sh *), Bash(bash */install.sh *), Bash(*/install.sh *), Bash(./install.sh *)
---
...
1. Read the source repo path from `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/rr/.source-repo`
...
2. If found:
   ...
   - Report the installed version after install completes (read from the freshly installed `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/rr/SKILL.md`)
