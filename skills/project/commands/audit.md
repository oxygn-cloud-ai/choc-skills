...
2. **Jira epic configured**: CLAUDE.md or GITHUB_CONFIG.md contains a CPT-<N> reference. This is a **blocking FAIL** — sessions cannot operate without epic scoping.
...
6. **Session Jira scoping**: Every `.claude/sessions/<role>.md` must contain a `## Jira Scoping Rule` section that references the project's epic key. Check each prompt file for the scoping section and verify the epic key matches CLAUDE.md. FAIL if any prompt is missing the scoping rule or references a different/no epic.
7. **Branch protection on main**: Derive `OWNER_REPO` from `git remote get-url origin | sed 's|.*github.com[:/]||; s|\.git$||'`, then `gh api "repos/$OWNER_REPO/branches/main/protection"` succeeds. SKIP for Non-Software if documented deviation.
8. **CI workflow exists** (Software only): `.github/workflows/test.yml` or similar. SKIP for Non-Software.
9. **GitHub Issues disabled**: `gh repo view --json hasIssuesEnabled --jq .hasIssuesEnabled` returns `false`. WARN if still enabled.
10. **No GitHub labels**: `gh label list` returns empty. WARN if labels exist (Jira is sole tracker).
11. **No stale worktree branches**: any `session/*` branch with no commits in >7 days → WARN
12. **Coverage thresholds** (Software only): if coverage job exists, thresholds match actuals. SKIP if no coverage.
...
Type: <Software|Non-Software>
  ...
  [FAIL] Jira epic not configured in CLAUDE.md (BLOCKING — sessions cannot operate)
  ...
  [FAIL] Session Jira scoping: 3/11 prompts missing ## Jira Scoping Rule section
  ...
  Result: 6 passed, 2 warnings, 3 failed, 1 skipped
