...
<objective>
Audit the current project against ~/.claude/MULTI_SESSION_ARCHITECTURE.md and ~/.claude/PROJECT_CONFIG.json. Report compliance gaps with PASS/FAIL/WARN/SKIP verdicts.
</objective>
...
- `test -f ~/.claude/PROJECT_CONFIG.json` — if missing: **STOP** with error: "~/.claude/PROJECT_CONFIG.json not found. This file is required for project auditing."

Read `~/.claude/MULTI_SESSION_ARCHITECTURE.md` for the full role list and requirements.
Read `~/.claude/PROJECT_CONFIG.json` for label, CI, branch protection, and doc requirements.
Read the project's `PROJECT_CONFIG.json` to understand project type and documented deviations.

## Step 3: Validate and determine project type

If `PROJECT_CONFIG.json` exists, run `scripts/validate-config.sh PROJECT_CONFIG.json` (or the installed copy) to validate it.

Read project type from JSON: `jq -r '.project.type' PROJECT_CONFIG.json`.
If `PROJECT_CONFIG.json` doesn't exist, infer: if `.github/workflows/` exists or `pyproject.toml`/`package.json` exists → Software. Else → Non-Software.
...
2. **Jira epic configured**: CLAUDE.md or PROJECT_CONFIG.json contains a CPT-<N> reference
